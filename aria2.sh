...
file="/home/lby/app/aria2"
aria2_conf="/home/lby/app/.aria2/aria2.conf"
aria2_log="/home/lby/app/.aria2/aria2.log"
...
Download_aria2_conf(){
	mkdir "${file}" && cd "${file}"
	wget --no-check-certificate -N "https://raw.githubusercontent.com/gitby2018/Linux-Bash/master/other/Aria2/aria2.conf"
...
Service_aria2(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/gitby2018/Linux-Bash/master/service/aria2_centos -O /etc/init.d/aria2; then
		...
		if ! wget --no-check-certificate  https://raw.githubusercontent.com/gitby2018/Linux-Bash/master/service/aria2_debian -O /etc/init.d/aria2; then
			echo -e "${Error} Aria2服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/aria2
...
Set_aria2_RPC_dir(){
	...
	read -e -p "(默认位置: /home/lby/download):" aria2_RPC_dir
	[[ -z "${aria2_RPC_dir}" ]] && aria2_RPC_dir="/home/lby/Download"
	echo
...
${Green_font_prefix}5.${Font_color_suffix} 如果你想在本地编辑配置文件，那么配置文件位置： ${Green_font_prefix}/home/lby/app/.aria2/aria2.conf${Font_color_suffix} (注意是隐藏目录) 。" && echo
...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/gitby2018/Linux-Bash/master/aria2.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	[[ -z ${sh_new_ver} ]] && echo -e "${Error} 无法链接到 Github !" && exit 0
	if [[ -e "/etc/init.d/aria2" ]]; then
		rm -rf /etc/init.d/aria2
		Service_aria2
	fi
	wget -N --no-check-certificate "https://raw.githubusercontent.com/gitby2018/Linux-Bash/master/aria2.sh" && chmod +x aria2.sh
